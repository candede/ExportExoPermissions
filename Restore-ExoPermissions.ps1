...
Write-Host "WhatIf Mode : $($WhatIfPreference -ne 'SilentlyContinue')"
...
# Cache: targetUPN -> @{ X500Addresses=[]; FullAccess=[]; SendAs=[]; SendOnBehalf=[]; FolderPerms={} }
$targetPermCache = @{}
...
function Get-ExistingMailboxPerms {
    ...
    $perms = @{
        ...
        SendAs        = @()
        SendOnBehalf  = @()
        FolderPerms   = @{}   # key = "FolderPath|DelegateUPN" -> $true
    }
    ...
    try {
        ...
        foreach ($emailAddress in @($mbxObj.EmailAddresses)) {
            $normalizedAddress = Normalize-X500Address -Address ([string]$emailAddress)
            if (-not [string]::IsNullOrWhiteSpace($normalizedAddress) -and $normalizedAddress -match '^(?i)X500:') {
...
foreach ($mbxRecord in $mailboxes) {
    ...
    if (Test-PermTypeRequested "SendOnBehalf") {
        ...
        foreach ($sob in $mbxRecord.MailboxDelegations.SendOnBehalf) {
            ...
            # Check if this delegate's target UPN is already in the list
            $alreadyPresent = $sobTargetList | Where-Object { $_ -like "*$delTgtUPN*" }

            if ($SkipExisting -and $alreadyPresent) {
                Write-LogEntry -TargetMailbox $tgtUPN -PermType "SendOnBehalf" `
                    -Delegate $delTgtUPN -Detail "-" -Status "Skipped" -Message "Already exists on target"
                continue
            }
            ...
            $anySOBAdded = $true
            Write-LogEntry -TargetMailbox $tgtUPN -PermType "SendOnBehalf" `
                -Delegate $delTgtUPN -Detail "-" `
                -Status $(if ($PSCmdlet.ShouldProcess($tgtUPN, "Stage SendOnBehalf for $delTgtUPN")) { "Applied" } else { "WhatIf" })
        }

        # Set-Mailbox takes the FULL list — apply once after collecting all trustees
        if ($anySOBAdded -and -not ($WhatIfPreference -ne 'SilentlyContinue')) {
            try {
                ...
                Write-LogEntry -TargetMailbox $tgtUPN -PermType "SendOnBehalf" `
                    -Delegate "(batch)" -Detail "List=$($sobTargetList -join ',')" `
                    -Status "Error" -Message "$_"
            }
        }
    }
